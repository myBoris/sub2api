		...
		if !skipBilling {
			...
			if subscription != nil {
				...
				if apiKey.Group != nil && apiKey.Group.IsPlusBalanceTier() && apiKey.User.PaidBalance <= 0 {
					AbortWithError(c, 403, "INSUFFICIENT_PAID_BALANCE", "Insufficient paid balance for plus group")
					return
				}
				if (apiKey.Group == nil || !apiKey.Group.IsPlusBalanceTier()) && apiKey.User.GiftBalance <= 0 {
					AbortWithError(c, 403, "INSUFFICIENT_GIFT_BALANCE", "Insufficient gift balance for free group")
					return
				}
			}
		}
